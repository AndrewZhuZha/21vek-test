...
**Последнее обновление:** 2026-06-26 — синхронизация с кодовой базой после исправлений Sprint 0.
...
- **Ранний init темы**: [js/portal-theme-init.js](../js/portal-theme-init.js) предотвращает FOUC.
...
### Недавно исправлено (2026-06-26)

| # | Было | Сделано |
|---|------|---------|
| 1 | Отсутствовал `portal-theme-init.js` → 404, FOUC | Создан [js/portal-theme-init.js](../js/portal-theme-init.js) |
| 2 | Три источника правды: HTML, `requestMap` в app.js, search-index | [data/request-types.json](../data/request-types.json) — единый источник; app.js использует `PortalRequestTypes`; сборка валидирует sync |
| 3 | README описывал sidebar; orphan CSS | README обновлён; `.sidebar-nav*` удалён из CSS |
| 4 | Карточки — `<div>` без клавиатуры | 21 карточка → `<button>`; `aria-label` в app.js; reset стилей в cards.css |

**Остаётся два источника контента услуг** (by design): разметка карточек в HTML (UI) + типы форм в JSON (логика). Связь контролируется сборкой — см. ARCH-02.

---

## 2. Архитектура и сопровождаемость

### ARCH-01 · ✅ Выполнено · portal-theme-init.js

**Было.** Файл подключён в `index.html` и `errors/*.html`, но отсутствовал в репозитории → 404, FOUC.

**Сделано.** Создан [js/portal-theme-init.js](../js/portal-theme-init.js): синхронно читает `localStorage`/cookie (`portal-theme`) и выставляет `data-theme` до отрисовки.

---

### ARCH-02 · ✅ Выполнено · Контракт добавления новой услуги

**Было.** Нет формализованного процесса; правки в нескольких местах без проверки.

**Сделано.** Зафиксировано в [README.md](../README.md) и ниже:

1. Добавить `<button class="service-card">` в [index.html](../index.html) с `data-request-type`, заголовком и описанием.
2. Добавить тип в [data/request-types.json](../data/request-types.json) (`title`, `options`, `defaultOpt`).
3. При необходимости — ключевые слова в [data/search.overrides.json](../data/search.overrides.json).
4. Пересобрать: `node scripts/build-search-index.mjs` (обновляет `search-index.js` и `request-types.js`).
5. Проверить: клик/Enter → форма; поиск по названию и синонимам.

**Файлы:** `index.html`, `data/request-types.json`, `data/search.overrides.json`, `js/search-index.js`, `js/request-types.js`.

---

### ARCH-03 · ✅ Выполнено · Валидация сборки search-index

**Было.** Сборка парсила HTML regex'ом; расхождения могли остаться незамеченными.

**Сделано.** [scripts/build-search-index.mjs](../scripts/build-search-index.mjs) вызывает `validateRequestTypesSync()`: каждый `data-request-type` из HTML должен присутствовать в `request-types.json` и наоборот. При несовпадении — `process.exit(1)`.

**Файлы:** `scripts/build-search-index.mjs`.

---

### ARCH-04 · ✅ Выполнено · Единый источник типов заявок

**Было.** `requestMap` захардкожен в `app.js`; сборка парсила его regex'ом.

**Сделано.**
- Источник: [data/request-types.json](../data/request-types.json)
- Runtime: [js/request-types.js](../js/request-types.js) → `window.PortalRequestTypes`
- [js/app.js](../js/app.js) читает `PortalRequestTypes`, не дублирует данные
- Сборка генерирует оба артефакта из одного JSON

**Файлы:** `data/request-types.json`, `js/request-types.js`, `js/app.js`, `scripts/build-search-index.mjs`.
...
**Проблема.** Модули общаются через `window.PortalConfig`, `PortalModal`, `PortalForm`, `PortalSearch`, `PortalSearchIndex`, `PortalRequestTypes` без формальной спецификации.
...
### UX-01 · ✅ Выполнено · Карточки услуг как интерактивные элементы

**Было.** `.service-card` — `<div>` без фокуса и клавиатурной активации.

**Сделано.** 21 карточка — `<button type="button" class="service-card">`; `aria-label` выставляется в [js/app.js](../js/app.js); reset стилей кнопки в [css/cards.css](../css/cards.css).
...
### UX-03 · ✅ Выполнено · Orphan CSS sidebar и README

**Было.** Sidebar удалён из разметки; orphan CSS и устаревший README.

**Сделано.** Удалены `.sidebar-nav*` из [css/nav.css](../css/nav.css) и [css/style.css](../css/style.css); [README.md](../README.md) описывает chip-навигацию и scroll-spy.
...
**Риск.** Непредсказуемый cascade при изменении порядка подключения stylesheet'ов.
...
### A11Y-01 · ✅ Выполнено · Клавиатурная активация карточек

См. **UX-01**. Карточки активируются Tab + Enter/Space.
...
|----|--------|---|---------|--------|----------|
| ARCH-01 | ✅ | P0 | Архитектура | S | portal-theme-init.js |
| STAB-01 | ✅ | P0 | Стабильность | S | Исправлен 404 theme init |
| UX-01 | ✅ | P0 | UX/a11y | M | Карточки как button |
| A11Y-01 | ✅ | P0 | a11y | — | Клавиатура карточек |
| ARCH-02 | ✅ | P1 | Архитектура | S | Контракт новой услуги |
| ARCH-03 | ✅ | P1 | Архитектура | S | Валидация сборки |
| ARCH-04 | ✅ | P2 | Архитектура | M | request-types.json |
| UX-03 | ✅ | P1 | UX | S | Sidebar orphan CSS + README |
| STAB-02 | ⏳ | P1 | Стабильность | S | Anti double-submit |
| STAB-03 | ⏳ | P1 | Стабильность | M | HTTP-слой Трекера |
| UX-02 | ⏳ | P1 | UX | S | Skip-link |
| UX-04 | ⏳ | P1 | UX | S | Consolidate duplicate CSS |
| A11Y-02 | ⏳ | P1 | a11y | S | Radio keyboard |
| A11Y-03 | ⏳ | P1 | a11y | S | Scroll-to-top a11y |
...
| ARCH-05 | ⏳ | P2 | Архитектура | S | npm scripts |
| UX-05 | ⏳ | P2 | UX | S | Inline search spinner |
| UX-06 | ⏳ | P2 | UX | S | Ctrl+F hint |
| UX-07 | ⏳ | P2 | UX | S | Mobile scroll-to-top |
| UX-08 | ⏳ | P2 | UX | S | Card text clipping |
| UX-09 | ⏳ | P2 | UX | S | FAQ placeholders |
| A11Y-04 | ⏳ | P2 | a11y | S | No alert() |
| A11Y-05 | ⏳ | P2 | a11y | M | color-mix fallback |
| STAB-04 | ⏳ | P2 | Стабильность | M | Browser smoke tests |
| STAB-05 | ⏳ | P2 | Стабильность | S | Environment URLs |
| UX-10 | ⏳ | P3 | UX | L | SVG icons |
| UX-11 | ⏳ | P3 | Design | M | Spacing/type tokens |
| UX-12 | ⏳ | P3 | UX | S | System theme default |
| ARCH-06 | ⏳ | P3 | Docs | S | Global contracts |
...
### Sprint 2 — UX и сопровождаемость
...
**Цель:** skip-link, CSS cleanup, a11y форм, prod URL.

### Backlog — P2/P3

UX-05…UX-12, A11Y-04/05, STAB-04/05, ARCH-05/06, TRK-04/05, CFG-03.
